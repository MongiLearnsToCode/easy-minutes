...
const PlanCard: React.FC<{
  ...
  return (
    <div className={`relative bg-white rounded-2xl p-6 sm:p-8 flex flex-col ${cardClasses} transition-transform duration-300`}>
      ...
      <h3 className="text-xl sm:text-2xl font-bold text-slate-800">{plan}</h3>
      <p className="mt-2 text-sm sm:text-base text-slate-500">{description}</p>

      <div className="mt-4 min-h-[80px] sm:min-h-[90px]">
        <div className="flex items-baseline">
          <span className="text-4xl sm:text-5xl font-extrabold tracking-tight text-slate-900">{price}</span>
          <span className="ml-1 text-lg sm:text-xl font-semibold text-slate-500">{pricePer}</span>
        </div>
        {monthlyBreakdown && <p className="text-sm sm:text-base text-slate-500 mt-1">{monthlyBreakdown}</p>}
        {savings && <p className="mt-1 text-xs sm:text-sm font-medium text-[#FF8A65]">{savings}</p>}
      </div>

      <ul className="my-6 sm:my-8 space-y-2 sm:space-y-3 text-sm sm:text-base text-slate-700 flex-grow">
      ...
      <a href="#" className={`w-full h-10 sm:h-11 mt-6 sm:mt-8 flex items-center justify-center text-center text-sm sm:text-base font-semibold py-3 px-4 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg ${buttonClasses}`}>
        {cta}
      </a>
    </div>
  );
...
const PricingPage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 h-full flex flex-col">
            <div className="flex-grow overflow-y-auto py-8 sm:py-12">
                <div className="text-center mb-8 sm:mb-12">
                    <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-800 px-4">Choose the plan that's right for you</h1>
                    <p className="mt-3 sm:mt-4 text-base sm:text-lg text-slate-500 max-w-2xl mx-auto px-4">Start for free, then unlock more features as you grow.</p>
                </div>

                <div className="flex justify-center items-center my-6 sm:my-10">
                    <span className={`px-3 sm:px-4 py-2 text-sm sm:text-base font-medium transition ${billingCycle === 'monthly' ? 'text-slate-800' : 'text-slate-500'}`}>Monthly</span>
                    <button
                    ...
                    <span className={`px-3 sm:px-4 py-2 text-sm sm:text-base font-medium transition ${billingCycle === 'annual' ? 'text-slate-800' : 'text-slate-500'}`}>
                        Annual
                        <span className="ml-1.5 sm:ml-2 text-[10px] sm:text-xs font-bold text-[#e65100] bg-[#ffccbc] rounded-full px-1.5 sm:px-2 py-0.5">Save up to 33%</span>
                    </span>
                </div>
                ...
                <div className="mt-8 sm:mt-12 pt-6 sm:pt-8 text-center border-t border-slate-200/80 max-w-3xl mx-auto px-4">
