...
const MeetingCard: React.FC<{ meeting: Meeting; onDelete: (id: string) => void; onView: (id: string) => void; onEdit: (id: string) => void; }> = ({ meeting, onDelete, onView, onEdit }) => (
    <div
        className="bg-white rounded-2xl shadow-lg shadow-slate-200/80 p-4 sm:p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:-translate-y-1 cursor-pointer"
        onClick={() => onView(meeting.id)}
    >
        <div>
            <div className="flex justify-between items-start">
                <h3 className="text-base sm:text-lg font-bold text-slate-800">{meeting.title}</h3>
            </div>
...
const MeetingListItem: React.FC<{ meeting: Meeting; onDelete: (id: string) => void; onView: (id: string) => void; onEdit: (id: string) => void; }> = ({ meeting, onDelete, onView, onEdit }) => (
    <div
        className="bg-white rounded-2xl shadow-lg shadow-slate-200/80 p-3 sm:p-4 flex items-center justify-between transition-all duration-300 hover:shadow-xl hover:border-slate-300 border border-transparent"
    >
        <div className="flex items-center gap-3 sm:gap-4 flex-grow cursor-pointer min-w-0" onClick={() => onView(meeting.id)}>
            <div className="flex-shrink-0 flex flex-col items-center justify-center bg-slate-100 rounded-lg p-1.5 sm:p-2 w-14 sm:w-20 text-center">
                ...
                <span className="text-xl sm:text-2xl font-bold text-slate-800">{new Date(meeting.date).getDate()}</span>
            </div>
            <div className="flex-grow min-w-0">
                <h3 className="text-base sm:text-lg font-bold text-slate-800 hover:text-[#FF8A65] transition-colors truncate">{meeting.title}</h3>
                <p className="mt-0.5 sm:mt-1 text-xs sm:text-sm text-slate-600 line-clamp-1">
                    {meeting.summary ? 
...
const DashboardPage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 h-full flex flex-col">
            <header className="flex flex-col md:flex-row md:items-center gap-3 md:gap-8 mb-6 sm:mb-8 flex-shrink-0">
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 flex-shrink-0">Your Meetings</h2>
                <div className="flex flex-col-reverse md:flex-row items-center gap-3 md:gap-4 w-full flex-grow">
                    <div className="relative w-full flex-grow">
                        <SearchIcon className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 sm:w-5 h-4 sm:h-5 text-slate-400 pointer-events-none" />
                        <input
                            type="text"
                            placeholder="Search meetings..."
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="bg-white border border-slate-200 rounded-full w-full pl-9 sm:pl-11 pr-3 sm:pr-4 py-2 sm:py-2.5 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-[#FF8A65] transition-all"
                        />
                    </div>
                    <div className="flex items-center self-end md:self-center space-x-1 bg-slate-200 p-1 rounded-full flex-shrink-0">
                        <button onClick={() => setViewMode('list')} className={`p-1.5 sm:p-2 rounded-full transition-colors ${viewMode === 'list' ? 'bg-white shadow-sm' : 'text-slate-500'}`}>
                            <ListIcon className="w-4 sm:w-5 h-4 sm:h-5" />
                        </button>
                        <button onClick={() => setViewMode('grid')} className={`p-1.5 sm:p-2 rounded-full transition-colors ${viewMode === 'grid' ? 'bg-white shadow-sm' : 'text-slate-500'}`}>
                            <GridIcon className="w-4 sm:w-5 h-4 sm:h-5" />
                        </button>
                         <button onClick={() => setViewMode('calendar')} className={`p-1.5 sm:p-2 rounded-full transition-colors ${viewMode === 'calendar' ? 'bg-white shadow-sm' : 'text-slate-500'}`}>
            ...
            <div className="flex-grow min-h-0 overflow-y-auto">
                {filteredMeetings.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center text-slate-500 bg-white rounded-2xl shadow-lg shadow-slate-200/80 p-6">
                        {searchQuery ? (
                             <>
                                <SearchIcon className="w-16 sm:w-20 h-16 sm:h-20 mb-4 text-slate-300" />
                                <h3 className="text-xl sm:text-2xl font-bold text-slate-700">No Results Found</h3>
                                <p className="mt-2 text-sm sm:text-base">Try adjusting your search terms to find your meeting.</p>
                            </>
                        ) : (
                            <>
                                <ListIcon className="w-16 sm:w-20 h-16 sm:h-20 mb-4 text-slate-300" />
                                <h3 className="text-xl sm:text-2xl font-bold text-slate-700">No Meetings Found</h3>
                                <p className="mt-2 text-sm sm:text-base">Create your first meeting summary on the Generator page.</p>
                            </>
                        )}
                    </div>
                )}
